...
function currentGoogleProps(): Partial<GoogleAuthProps> | undefined {
  return getMcpAuthContext()?.props as Partial<GoogleAuthProps> | undefined;
}
 ...
 * `env` is closed over from the Worker's fetch() call (see src/index.ts) —
 * it is NOT available inside the factory context object itself.
 */
export function createServer(env: GoogleClientEnv) {
      ...
      return jsonContent({
        ...
        connectedServices: {
          youtube: youtubeConnected
            ? { connected: true, account: props?.email ?? "unknown" }
            : { connected: false },
          facebook: { connected: false, note: "Planned Phase 3" },
          instagram: { connected: false, note: "Planned Phase 3" },
          shopify: { connected: false, note: "Planned Phase 4" },
          amazon: { connected: false, note: "Planned Phase 5" },
        },
  ...
  server.registerTool(
    ...
    async ({ maxResults, pageToken }) => {
      ...
      if (!uploadsPlaylistId) {
        return jsonContent({ items: [], note: "No uploads playlist found for this channel." });
      }
      ...
      const data = await youtubeApiFetch("playlistItems", token, params);
      return jsonContent(data);
    },
  );
  ...
  server.registerTool(
    "youtube_get_video_details",
    {
      ...
      inputSchema: {
        videoIds: z.array(z.string()).min(1).max(50).describe("YouTube video IDs to look up"),
      },
    },
    async ({ videoIds }) => {
      const props = requireGoogleAuth();
  ...
  server.registerTool(
    ...
    {
      description: "List the authenticated user's YouTube playlists. Read-only.",
      inputSchema: {
        maxResults: z
          .number()
          .int()
    ...
    async ({ maxResults, pageToken }) => {
      ...
      if (pageToken) params.pageToken = pageToken;
      const data = await youtubeApiFetch("playlists", token, params);
      return jsonContent(data);
    },
  );
  ...
  server.registerTool(
    ...
    {
      description: "List the videos inside a specific YouTube playlist. Read-only.",
      inputSchema: {
  ...
  server.registerTool(
    "youtube_get_channel_analytics",
    {
      ...
      inputSchema: {
        ...
        dimensions: z.string().optional().describe("Comma-separated dimensions, e.g. 'day'"),
      },
    },
    async ({ startDate, endDate, metrics, dimensions }) => {
      ...
      const data = await youtubeAnalyticsFetch(token, params);
      return jsonContent(data);
    },
  );
